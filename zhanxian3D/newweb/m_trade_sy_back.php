...
class Trade_syInfo
{
    ...
    {
        $this->dbh = new PDO(PDO_Trade_syInfo_host,PDO_Trade_syInfo_root,PDO_Trade_syInfo_pass);
...
if( $ZONE ) $dowhere .= " AND ZONE IN($ZONE) ";
        ...
        switch ($PF) {
            case 'qqgame':
                $dowhere .= " AND PF = 'qqgame' " ;
                break;   
            case 'qzone':
                $dowhere .= " AND PF = 'qzone' " ;
                break;
            case 'other':
                $dowhere .= " AND PF != 'qqgame' AND PF != 'qzone' " ;
                break;
        }
        $stmt = $this->dbh->prepare('SELECT NUM*PRICE AS SUM_YB, (AMOUNT) AS SUM_AMOUNT, FROM_UNIXTIME(TIME) AS YMD, PF, ACC, NAME, ZONE FROM TRADE WHERE STATUS = 1 ' . $dowhere . $orderby . ' ' . $limit);
...
if( $ZONE ) $dowhere .= " AND ZONE IN($ZONE) ";
        ...
        $stmt = $this->dbh->prepare('SELECT COUNT(ID) AS total FROM TRADE WHERE STATUS = 1 ' . $dowhere );
    ...
    public function acc_group_list( $where=array('ZONE'=>'', 'ACC'=>'', 'TIME1'=>'', 'TIME2'=>'','PF'=>''), $orderby='', $limit='LIMIT 1' )
    {
        ...
        $stmt = $this->dbh->prepare('SELECT SUM(NUM*PRICE) AS SUM_YB, SUM(AMOUNT) AS SUM_AMOUNT, FROM_UNIXTIME(TIME) AS YMD, PF, ACC, NAME, ZONE FROM TRADE WHERE STATUS = 1  ' . $dowhere . ' GROUP BY ACC ' . $orderby . ' ' . $limit);
        #if( $ZONE ) $stmt->bindParam(':ZONE',$ZONE);
        if( $ACC ) $stmt->bindParam(':ACC',$ACC);
        if( $TIME1 ) $stmt->bindParam(':TIME1',$TIME1);
        if( $TIME2 ) $stmt->bindParam(':TIME2',$TIME2);
...
if( $ZONE ) $dowhere .= " AND ZONE IN($ZONE) ";
        ...
        $stmt = $this->dbh->prepare('SELECT count(distinct ACC) AS total FROM TRADE WHERE STATUS = 1  ' . $dowhere );
...
if( $ZONE ) $dowhere .= " AND ZONE IN($ZONE) ";
        ...
        $stmt = $this->dbh->prepare('SELECT SUM(NUM*PRICE) AS SUM_YB, SUM(AMOUNT) AS SUM_AMOUNT, FROM_UNIXTIME(TIME) AS YMD, PF, ACC, NAME, ZONE FROM TRADE WHERE STATUS = 1 ' . $dowhere . ' GROUP BY ZONE,ACC ' . $orderby . ' ' . $limit);
...
if( $ZONE ) $dowhere .= " AND ZONE IN($ZONE) ";
        ...
        $stmt = $this->dbh->prepare('SELECT count(distinct ZONE,ACC) AS total FROM TRADE WHERE STATUS = 1  ' . $dowhere );
    ...
    {
        $domians = trim( $domian, ',' );
        $stmt = $this->dbh->prepare("SELECT SUM(AMOUNT) AS total FROM TRADE WHERE STATUS = 1  AND ZONE IN (:zone) "); //[Q战神、焚天]
