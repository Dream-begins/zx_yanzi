...
if (isset($_GET['to']) && !empty($_GET['to'])) {
   ...
   $sql=' WHERE '.$sql.' TYPE=2'; 
   $sql    = "SELECT from_unixtime(TIME,'%Y-%m-%d %H:%i:%S') as datetime,ZONEID,SERVERID,USERID,ACCNAME,NAME,LEVEL,CHARGEMOUNT,REDUCEDevote,ADDMount,NOWMount,EVENT,ITEMID,ITEMNAME,PARA1,PARA2,PARA3,PARA4,PARA5,EXTRA from log_currency $sql order by datetime desc";
    ...
    require_once "../exportExcel.php";
       export(array("日期","区","服务器","玩家id","账号名","玩家名","玩家等级","充值金额","消耗帮贡","帮贡增加量","帮贡现存量","货币事件","物品id","物品名","参数1","参数2","参数3","参数4","参数5","额外信息"),$rows,"帮贡统计");
}else{
$page=$_POST['page'];
$pageSize=$_POST['rows'];
