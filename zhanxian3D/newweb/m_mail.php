...
class MAILInfo
{
    ...
    public function get_list($limit='limit 1')
    {
        $sql = "SELECT ID,STATE,FROMZONE,FROMNAME,TOZONE,TONAME,
                       ...
                       `TEXT`,SENDMONEYNUMA,SENDMONEYNUMB,TOID,FROMID,BIN 
                       FROM `MAIL` where 1=1 $limit ";
     ...
     * @desc 获取MAIL表总数
     */
    public function get_total($start)
    {
        if($start){
            $end =  $start+86400;
            $limit = "where CREATETIME>={$start} AND CREATETIME<={$end}";
        }else{
            $limit = "";
        }
        $sql = "SELECT COUNT(1) AS nu FROM `MAIL`$limit";
        $stmt = $this->dbh->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetch();
     ...
     */
    public function get_total_name($name,$start)
    {   if($start){
            $end =  $start+86400;
            $limit = "AND CREATETIME>={$start} AND CREATETIME<={$end}";
         }else{
            $limit = "";
         }

        $sql = "SELECT COUNT(1) AS nu FROM `MAIL` WHERE TONAME = :name $limit";
        $stmt = $this->dbh->prepare($sql);
        $stmt->bindParam(':name', $name);
        $stmt->execute();
        $result = $stmt->fetch();
