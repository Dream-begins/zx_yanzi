...
for ($i=$zone1; $i<=$zone2; $i++) 
{
		...
		if(!$dburl)
		  ...
		  mysql_select_db(getDBName($i), $con) or die("mysql select db error". mysql_error());
		  mysql_query("set names 'utf8'",$con);
		  //	mysql_query("SET NAMES utf8",$con); 	
	if ($isadd == 1)
	$sql = "insert into `SHOPLIMIT`(ICON,TITLE,WEIGHT,PAGE,INDEXID,OBJID,OBJNAME,SUPERMARKETPOS,MONEYTYPE,ORIGINALPRICE,DISCONTPRICE,ISBIND,SINGLECANBUYNUM,TOTALBUYLIMITNUM,OPENTYPE,OPENTTIME,CLOSETIME,LIMITEDPURCHASESTARTTIME,LIMITEDPURCHASEENDTIME,TAGTYPE,NEEDVIPLEVEL) 
		values('".$tabIcon."','".$tabTitle."','".$weight."','".$page."','".$uniqid."','".$objid."','".$objname."','".$supermarketpos."','".$moneytype."','".$originalprice."','".$discontprice."','".$isbind."','".$singlecanbuynum."',	'".$totalbuylimitnum."'
		,'".$opentype."','".$openttime."','".$closetime."','".$limitedpurchasestarttime."','".$limitedpurchaseendtime."','".$tagtype."','".$needviplevel."')";
	else if($isadd == 2)
