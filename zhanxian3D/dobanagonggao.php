...
$con = mysql_connect(FT_MYSQL_ZONE_MSG_HOST,FT_MYSQL_ZONE_MSG_ROOT,FT_MYSQL_ZONE_MSG_PASS) or exit('<script>$("#monitor_check").prop("checked",false);</script>'."<font color='red'>error:数据库连接失败</font><br>".mysql_error());
...
if($action == 'jinyan')
{
  $timesss = isset($_GET['timesss']) ? $_GET['timesss'] : '';

  $seconds = $timesss * 3600;

  $gm_code  = trim(h_post_param("gm_code",''));
  $gm_zones = (int)h_post_param("gm_zones",'');
  ...
  $ServerID = $value;
  $Command  = "notalk name={$Command} time={$seconds}";
...
if($action == 'jiechujinyan')
{
  ...
  foreach ($zones_arr as $key => $value) 
  {
    ...
    $SN = md5(uniqid(rand(), true));
  $ServerID = $value;
  $Command  = "talk name={$Command} zone={$ServerID}";
  $values .= "('{$SN}','{$FromIP}','{$ServerID}','{$Command}','1','{$OptTimeStamp}','{$FlagId}','{$CmdLength}','{$admin_name}'),";
  }
...
if($action == 'jiechufenghao')
{
  ...
  foreach ($zones_arr as $key => $value) 
  {
    ...
    $SN = md5(uniqid(rand(), true));
  $ServerID = $value;
  $Command  = "free name={$Command} zone={$ServerID}";
  $values .= "('{$SN}','{$FromIP}','{$ServerID}','{$Command}','1','{$OptTimeStamp}','{$FlagId}','{$CmdLength}','{$admin_name}'),";
  }
...
if($action == 'gonggao')
{
  ...
  $FromIP   = isset($_SERVER["REMOTE_ADDR"]) ? $_SERVER["REMOTE_ADDR"] : ''; 
  $Command  = "announce " . ltrim($gm_code,'/');
...
if($action == 'quxiaomimabaohu')
{
  ...
  foreach ($zones_arr as $key => $value) 
  {
    ...
    $SN = md5(uniqid(rand(), true));
  $ServerID = $value;
  $Command  = "cancelpassword name={$Command} ";
  $CmdLength = strlen($Command);
  $values .= "('{$SN}','{$FromIP}','{$ServerID}','{$Command}','1','{$OptTimeStamp}','{$FlagId}','{$CmdLength}','{$admin_name}'),";
  }
