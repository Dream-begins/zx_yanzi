...
if($action == "zone_list")
{
    ...
    $ymd = strtotime($ymd);
    $ymd = date('Y_m_d', $ymd);
    $appver = trim($_POST['appVer']) ;
    $msg = trim($_POST['msg']) ;

    $where_serach = ' where 1=1 ';
    if($appver){ $where_serach .=" and appVer = '".$appver."'"  ;}
    if($msg){ $where_serach .=" and msg not like '%{$msg}%'" ;}
    $page = (int)$_POST['page'];
    if($page <= 0) {
        $page = 1;
    }
    ...
    $stmt = $dbh->prepare("SELECT * FROM int_error_inside_{$ymd} " .$where_serach. $orderby . $limit);
    ...
    $stmt = $dbh->prepare("SELECT COUNT(*) AS total FROM int_error_inside_{$ymd} ".$where_serach);
